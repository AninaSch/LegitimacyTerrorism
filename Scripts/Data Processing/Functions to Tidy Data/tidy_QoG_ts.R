...
tidy_QoG_ts <- function(path_loadoriginal, path_savetidy){
  
  #       1. read Polity Data
  print("importing Penn data... ")
  QoG <- rio::import(path_loadoriginal)
  QoG <- rio::import("../../../Data/Original Data/QoG_QualityOfGovernment/qog_std_ts_jan20.csv") # for debugging
  ...
  QoG_select <- QoG %>%
    select(
      ...
      ciri_physint,ciri_polpris,ciri_speech,ciri_tort,
      cpds_tg, cpds_vt,
      bci_bci, bci_bcistd,
      icrg_qog,
...
QoG_ts_tidy <- QoG_select %>%
  ...
  saveRDS(QoG_ts_tidy, file = path_savetidy)
  saveRDS(QoG_ts_tidy, file = "../../../Data/Processed Data/QoG_ts_tidy.rds")
  
  print("processed QoS_ts data saved")
